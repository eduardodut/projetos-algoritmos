...
export Arvore_Binaria_3D, proximo_item!
...
function max_heapify!(
    ...
    while (c = 2 * primeiro - 1) < ultimo
        ...
        if c < ultimo && vetor_entrada[c].raiz < vetor_entrada[c+1].raiz
            #aumenta c em 1 para referenciar a próxima posição, para indicar que o elemento
            c += 1
        end
        if vetor_entrada[primeiro].raiz < vetor_entrada[c].raiz
            # println("antes: ",vetor_entrada[c]," ",  vetor_entrada[primeiro])
            troca_elementos!(vetor_entrada, c, primeiro)
            # println("antes: ",vetor_entrada[primeiro]," ",  vetor_entrada[c])
            primeiro = c
        else
            break
        end
